...
describe('koh-vibe-statusline', () => {
  ...
  it('writes nothing of its own on stdout — that stream belongs to the statusline', () => {
    expect(run(PAYLOAD)).toBe('');
  });

  it('lets the delegate output through, and passes it the SAME input back', () => {
    const out = run(PAYLOAD, b64(`/usr/bin/head -c 12`));
    expect(out).toBe(PAYLOAD.slice(0, 12));
    // And the snapshot was indeed captured along the way.
    expect(readFileSync(join(home, 'status.json'), 'utf8')).toBe(PAYLOAD);
  });

  it('survives a delegate that fails, without returning anything noisy', () => {
    expect(run(PAYLOAD, b64('/bin/sh -c "exit 3"'))).toBe('');
    expect(existsSync(join(home, 'status.json'))).toBe(true);
  });

  it('accepts a delegate whose command contains quotes and apostrophes', () => {
