...
describe('focusPlanFor', () => {
  it('reveals a vscode session panel, by its identifier', () => {
  ...
  it('opens nothing either for sdk and unknown', () => {
    expect(focusPlanFor({ ...base, origin: 'sdk' }).kind).toBe('explain');
    expect(focusPlanFor({ ...base, origin: 'unknown' }).kind).toBe('explain');
  });
});

// `focusPlan` is the single rule of which `focusPlanFor` (local path, a
// full `Session`) and the broker (remote path, only sessionId / origin /
// label read from a request file) are two callers — never two copies of
// the same decision.
describe('focusPlan — the rule shared by both paths', () => {
  it('reveals by identifier for vscode or desktop', () => {
  ...
  it('explains for any other origin, including a missing one (a request from an earlier version)', () => {
  ...
  it('focusPlanFor(session) delegates to focusPlan, it does not encode its own rule', () => {
