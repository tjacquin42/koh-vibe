...
describe('reduce', () => {
  ...
  it('finishes unread, then idle after acknowledgment', () => {
    const a = reduce(undefined, ev('UserPromptSubmit'));
  ...
  it('an Ack on a running session changes nothing', () => {
    const a = reduce(undefined, ev('PreToolUse', { toolName: 'Bash' }));
    expect(reduce(a, ev('Ack'))?.status).toBe('running');
  });

  it('SessionEnd keeps the session, marked finished and idle', () => {
    const a = reduce(undefined, ev('SessionStart'));
  ...
  it('a late hook does not revive a finished conversation, but keeps its cumulative effects', () => {
    const a = reduce(undefined, ev('SessionStart'));
  ...
  it('a late SessionEnd does not finish a conversation that has spoken since (a twin in another editor)', () => {
    const a = reduce(undefined, ev('SessionStart'));
  ...
  it('an Ack on an absent session creates nothing: only a hook event can bring a session into being', () => {
    expect(reduce(undefined, ev('Ack'))).toBeUndefined();
  });

  it('a late event does not regress the status', () => {
  ...
  it('but its cumulative effects still count', () => {
    const a = reduce(undefined, ev('Stop'));
  ...
  it('keeps track of several sessions and a worktree', () => {
    const map = reduceAll([
