...
describe('releaseLabel', () => {
  ...
  it('does not invent a gap when the count is missing or is not one', () => {
  ...
  it('without a tag, there is no version to display', () => {
...
describe('buildCommit', () => {
  ...
  it('marks with a star a package that does not match its commit', () => {
    expect(buildCommit({ commit: '1736ec0', dirty: true })).toBe('1736ec0*');
  });

  it('marks only on a real boolean: a dubious value does not warrant an alert', () => {
    expect(buildCommit({ commit: '1736ec0', dirty: 'oui' })).toBe('1736ec0');
    expect(buildCommit({ commit: '1736ec0' })).toBe('1736ec0');
  });

  it('without a commit, there is nothing to display — not even a star', () => {
...
describe('versionLabel', () => {
  it('juxtaposes the shipped version and the commit', () => {
  ...
  it('says that no version has been shipped rather than borrowing one', () => {
    expect(versionLabel({ commit: '1736ec0', dirty: false })).toBe('no version · 1736ec0');
  });

  it('stays displayable with no timestamp at all', () => {
    expect(versionLabel(undefined)).toBe('no version');
  });
});

describe('readBuildStamp', () => {
  it('reads the file placed at the package root', async () => {
